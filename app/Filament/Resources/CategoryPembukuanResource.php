...
class CategoryPembukuanResource extends Resource
{
    ...
    protected static ?string $pluralModelLabel = 'Kategori Pembukuan';

    public static function form(Form $form): Form
    {
        return $form
            ->schema([
                Forms\Components\TextInput::make('category_pembukuan')
                    ->label('Kategori Pembukuan')
                    ->required()
                    ->maxLength(255)
                    ->placeholder('Masukkan Kategori Pembukuan'),
            ]);
    }
    ...
    public static function table(Table $table): Table
    {
        return $table
            ->columns([
                ...
                Tables\Columns\TextColumn::make('category_pembukuan')
                    ->label('Kategori Pembukuan')
                    ->searchable()
                    ->sortable(),
            ])
